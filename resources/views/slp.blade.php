	...
	<div class="main-content">
		<div class="row small-spacing">
			<div class="col-xs-12">
             ...
             @foreach($slp as $Slp)
							<tr >
                <td class="table-content"><img src="http://latitudslp.com/{{$Slp->img}}" alt="" data-nsfw-filter-status="sfw" class="news-img"></td>
                <td class="table-content">{{$Slp->title}}</td>
                <td class="news-content">{{$Slp->content}}</td>
								<td class="table-content">{{$Slp->updated_at}}</td>
								<td class="table-content">
                  <ul class="list-inline">
                    <li class="margin-bottom-10"><button type="button" class="btn btn-social waves-effect waves-light btn-twitter" onclick="window.location.href='/news-detail?id={{$Slp->id}}'"><i class="mdi mdi-eye"></i></button></li>
                    <li class="margin-bottom-10"><button type="button" class="btn btn-social waves-effect waves-light btn-youtube" data-remodal-target="newsremove" data-id = {{$Slp->id}} id="data" onclick="NewsRemove(this)"><i class="mdi mdi-delete"></i></button></li>
                  </ul>
                </td>
              </tr>
              @endforeach
