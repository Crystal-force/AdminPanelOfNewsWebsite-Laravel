		...
		<div class="row small-spacing">
			<div class="col-xs-12">
              ...
              @foreach($all_users as $AllUsers)
							<tr >
                <td class="table-content"><img src="http://latitudslp.com/{{$AllUsers->avarter}}" alt="" data-nsfw-filter-status="sfw" ></td>
								<td class="table-content">{{$AllUsers->email}}</td>
								<td class="table-content">{{$AllUsers->name}}</td>
								<td class="table-content">{{$AllUsers->created_at}}</td>
								<td class="table-content">
                  <p style="color:blue">pending</p>
                </td>
								<td class="table-content">
                  <ul class="list-inline">
                    <li class="margin-bottom-10"><button type="button" class="btn btn-social waves-effect waves-light btn-twitter" onclick="window.location.href='/alluser-detail?id={{$AllUsers->id}}'"><i class="mdi mdi-eye"></i></button></li>
                    <li class="margin-bottom-10"><button type="button" class="btn btn-social waves-effect waves-light btn-youtube" data-remodal-target="remodal_1" data-id = {{$AllUsers->id}} id="data" onclick="UserRemove(this)"><i class="mdi mdi-delete"></i></button></li>
                  </ul>
                </td>
              </tr>
              @endforeach
